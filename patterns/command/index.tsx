    ...
    <div className="not-prose" style={style}>
      <div className="mt-5">
        {showUndo && (
          <button
            onClick={onUndo}
            className="bg-orange-700 text-white px-3 rounded-lg text-lg cursor-pointer font-bold"
          >
            UNDO
          </button>
        )}
      ...
      <ul className="w-full text-2xl mt-5">
        {list.map((i) => (
          <li
            className="py-2 flex justify-between border-2 px-4 rounded-lg items-center mb-4"
            key={i.key}
          >
            <span
              className={`font-bold ${
                i.status === "finished" ? "line-through text-gray-400" : ""
              }`}
            >
              {i.todo}
            </span>
            <div className="text-lg">
              ...
              <span
                className="ml-5 cursor-pointer text-pink-500 font-bold"
                onClick={() => onFinish(i.key)}
              >
                FINISH
              </span>
            </div>
